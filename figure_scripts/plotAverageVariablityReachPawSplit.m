function plotAverageVariablityReachPawSplit(learner_summary,nonLearner_summary,learning_summary)

% plots the average trajectory variability of the paw dorsum (avg. distance
% from avg. session trajectory) during reach for each session for learners,
% non-learners, and all rats combined

distData = learner_summary.mean_pd_dist_from_traj_reach;    % learners data
distDataNL = nonLearner_summary.mean_pd_dist_from_traj_reach;   % non-learners data
distDataAll = learning_summary.mean_pd_dist_from_traj_reach;    % all rat data
...
avgDistTrajAll = squeeze(nanmean(distDataAll,2));   % average for all rats
...
line(1:10,avgDistSessionAll,'Color','k','lineWidth',5)  % plot average all rats
hold on     % plot learners data
