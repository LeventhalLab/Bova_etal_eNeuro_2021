function plotAverageVariablityReachDigSplit(learner_summary,nonLearner_summary,learning_summary)

% plots average reach trajectory variability of digit 2 across sessions
% (learners, non-learners, and all rats combined)

distData = learner_summary.mean_dig_dist_from_traj_reach(:,:,:,2);  % pull out data for digit 2
distDataNL = nonLearner_summary.mean_dig_dist_from_traj_reach(:,:,:,2);
allData = learning_summary.mean_dig_dist_from_traj_reach(:,:,:,2);
...
avgAllTraj = squeeze(nanmean(allData,2));   % calculate average for all rats combined
...
line(1:10,avgAllSess,'Color','k','lineWidth',5) % plot average all rats
hold on
