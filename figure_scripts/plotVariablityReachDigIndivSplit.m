function plotVariablityReachDigIndivSplit(learner_summary,nonLearner_summary,group)

% plots individual rats' average digit 2 reach trajectory variability over
% sessions (learners (group = 1) and non-learners (group = 2) plotted separately)

if group == 1   % pull out data, set colors 
	data = learner_summary.mean_dig_dist_from_traj_reach(:,:,:,2);
    plot_color = {[0/255 102/255 0/255] [95/255 130/255 226/255] [216/255 85/255 116/255] [212/255 216/255 85/255]};
else
    data = nonLearner_summary.mean_dig_dist_from_traj_reach(:,:,:,2);
...
plotData = squeeze(nanmean(data,2));    % calculate avg. traj. variability (mean dist. from avg. traj)
...
ylabel({'mean dist. from'; 'avg. traj. (mm)'})
xlabel('day number')
set(gca,'xlim',[.5 10.5]);
