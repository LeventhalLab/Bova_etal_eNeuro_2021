function detVal = calculateDetVarMatrixfromPaw

% calculates average determinant of the covariance matrixes for digit
% endpoint positions subtracted from paw dorsum position 

dataDir = '/Volumes/DLC_data/rat kinematic summaries';  % location of individual rat kinematic summaries
cd(dataDir)

ratFiles = dir('R*ary.mat');   

script_findLearners     % separate "learners" and "non-learners"

for i_rat = 1:length(ratFiles)  % load in each rats covariance data
    ...
    for i_sess = 1:10
        ...
        meanDig1End(:,i_sess,i_rat) = squeeze(ratSummary.mean_dig_endPts_reach_fromPaw(i_sess,1,1,:));
        meanCovDig1(:,:,i_sess,i_rat) = squeeze(ratSummary.cov_dig_endPts_reach_fromPaw(i_sess,1,1,:,:));
        meanDig2End(:,i_sess,i_rat) = squeeze(ratSummary.mean_dig_endPts_reach_fromPaw(i_sess,1,2,:));
        meanCovDig2(:,:,i_sess,i_rat) = squeeze(ratSummary.cov_dig_endPts_reach_fromPaw(i_sess,1,2,:,:));
        meanDig3End(:,i_sess,i_rat) = squeeze(ratSummary.mean_dig_endPts_reach_fromPaw(i_sess,1,3,:));
        meanCovDig3(:,:,i_sess,i_rat) = squeeze(ratSummary.cov_dig_endPts_reach_fromPaw(i_sess,1,3,:,:));
        meanDig4End(:,i_sess,i_rat) = squeeze(ratSummary.mean_dig_endPts_reach_fromPaw(i_sess,1,4,:));
        meanCovDig4(:,:,i_sess,i_rat) = squeeze(ratSummary.cov_dig_endPts_reach_fromPaw(i_sess,1,4,:,:));
    end
    
    switch pawPref  % make consistent between left and right pawed rats
        case 'left'
...
% calculate averages for paw and digits
...
% separate det. of cov. matrix data between "learners" (1) and
% "non-learners" (2)
nl_rats = 1 : 14;
nl_rats(learningRats) = [];

detVal(1).paw = detValPd(learningRats,:);
