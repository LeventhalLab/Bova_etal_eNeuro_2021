function plotAvgDorsumEndptCoorSplit(learner_summary,nonLearner_summary,i_coor,learning_summary)

% plot average endpoint of paw dorsum in x, y, or z direction (set by
% i_coor) for learners, non-learners, and all rats combined

lData = learner_summary.mean_pd_endPt_reach(:,:,i_coor); % pull out endpoint data from selected coordinate direction
nlData = nonLearner_summary.mean_pd_endPt_reach(:,:,i_coor);
allData = learning_summary.mean_pd_endPt_reach(:,:,i_coor);

if i_coor == 3 || i_coor == 2   % flip z and y axes to be consistent with coordinate direction standards
    lData = lData*-1;
    nlData = nlData*-1;
    allData = allData*-1;
end
...
e = errorbar(1:10,avgL,errbarsL,'linestyle','none');   % add error bars
e.Color = avgColor;
scatter(1:10,avgNL,avgMarkerSize,'filled','MarkerEdgeColor',avgColorNL,'MarkerFaceColor',avgColorNL);  % plot average non-learners
e1 = errorbar(1:10,avgNL,errbarsNL,'linestyle','none');   % add error bars
e1.Color = avgColorNL;
