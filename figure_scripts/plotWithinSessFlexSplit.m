function plotWithinSessFlexSplit(slidingWindowKinematics,session,num_bins)
...
for i_rat = learningRats
    flexData_l(:,ratCt) = slidingWindowKinematics(i_rat).flexion_reach_end(:,session,:);
    ratCt = ratCt+1;
end 

ratCt = 1;  % pull out data non-learners
for i_rat = nl_rats
    flexData_n(:,ratCt) = slidingWindowKinematics(i_rat).flexion_reach_end(:,session,:);
    ratCt = ratCt+1;
end 

data_l = NaN(num_bins,num_ratsL);   % trajectory variability data
data_n = NaN(num_bins,num_ratsN);

for i_rat = 1 : num_ratsL    % pull out data into new structure for plotting
    for i_trial = 1:num_bins    
        data_l(i_trial,i_rat) = flexData_l(i_trial,i_rat);
        
        if isnan(data_l(i_trial,i_rat)) % if rat drops out (i.e. no more trials) carry last score forward
            lastDataPt = ~isnan(flexData_l(:,i_rat)); 
            rowNum = find(lastDataPt == 1,1,'last'); % find last data point
            if isempty(rowNum)
                continue
            else
            data_l(i_trial,i_rat) = flexData_l(rowNum,i_rat); % set current trial to last data point
            end
        end        
    end
end
...
for i_rat = 1 : num_ratsN    % pull out data into new structure for plotting
    for i_trial = 1:num_bins    
        data_n(i_trial,i_rat) = flexData_n(i_trial,i_rat);
        
        if isnan(data_n(i_trial,i_rat)) % if rat drops out (i.e. no more trials) carry last score forward
            lastDataPt = ~isnan(flexData_n(:,i_rat)); 
            rowNum = find(lastDataPt == 1,1,'last'); % find last data point
            if isempty(rowNum)
                continue
            else
            data_n(i_trial,i_rat) = flexData_n(rowNum,i_rat); % set current trial to last data point
            end
        end        
    end
end
...
% figure properties
ylabel({'\delta at'; 'reach end (degrees)'},'FontSize',10)
xlabel(session)
set(gca,'xlim',[.5 num_bins+.5]);
set(gca,'ylim',[15 35]);
set(gca,'ytick',[15 25 35],'xtick',[30]);
set(gca,'FontSize',10);
box off