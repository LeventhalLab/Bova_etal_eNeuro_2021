...
% -     Parent directory
% o     Rat folder, named with rat identifier (e.g., "R0186"ù)
% ÔÇ    Sessions folders RXXXX_YYYYMMDDz (e.g., "R0186_20170921a" would be the first session recorded on September 21, 2017 for rat R0186)
% 
...
% -     Parent directory
% o     Year (e.g., "ò2018"ô)
% Ô     YYYYMM_calibration (e.g., "201810_calibration"ô would contain calibration images/files for October, 2018)
...
% ‚Ä¢	YYYYMM_calibration_files - calibration files. These are .mat files containing fundamental, essential matrices, etc.
% ‚Ä¢	YYYYMM_manually_marked - calibration images that have been manually marked in Fiji, as well as .csv files containing checkerboard corner coordinates
% ‚Ä¢	YYYYMM_original_images - original calibration images
%
...
calImageDir = '/Volumes/DLC_data/calibration_images';
...
for i_rat = 1 : numRatFolders
    ...
    sessionsFilename = sprintf('%s_sessions.csv',ratID); % load in sessions info for rat 
    sessionsCSV = fullfile(ratRootFolder,sessionsFilename);
    sessionsInfo = readtable(sessionsCSV);

    existBoxInfo = strcmp('boxNumber',sessionsInfo.Properties.VariableNames);
    if ~any(existBoxInfo == 1) % for current project, only analyzing rats that I added box number info to sessions csv file (just makes it easier)
        continue
    end 
    ...
    switch ratID
        case 'R0216'
            startSession = numSessions - (numSessions-1);
    ...
    for iSession = startSession : 1 : endSession
        ...
        datesInTable = num2str(sessionsInfo.date);
        rowInSessions = strmatch(sessionDate,datesInTable);
        
        if isempty(sessionsInfo.boxNumber(rowInSessions))   
            continue
        end
        
        boxNum = sessionsInfo.boxNumber(rowInSessions);
