...
% -	Parent directory
% o	Rat folder, named with rat identifier (e.g., "R0186"�)
% 	Sessions folders RXXXX_YYYYMMDDz (e.g., "R0186_20170921"� would be the first session recorded on September 21, 2017 for rat R0186)
...
% for saving a backup to a shared drive %% could delete
sharedX_DLCoutput_path = '';
...
for i_rat = 1 : numRatFolders   % change limits to work on specific rats
    ...
    for iSession = startSession : endSession
        ...
        for iFile = 1 : length(calFileList)  
            calBoxNum = textscan(calFileList(iFile).name,'SR_boxCalibration_box%2c.mat');
            calBoxNum = str2double(calBoxNum);
            C = textscan(calFileList(iFile).name,'SR_boxCalibration_box%11c.mat');   
            calDateList{iFile} = C{1};
            
            if calBoxNum == 1
                D = textscan(calFileList(iFile).name,'SR_boxCalibration_box01_%8c.mat'); 
            elseif calBoxNum == 2
                D = textscan(calFileList(iFile).name,'SR_boxCalibration_box02_%8c.mat'); 
            end 
            calDateNums(iFile) = str2double(D);
        end
        ...
        if ~any(existBoxInfo == 1) % for current project, only analyzing rats that I added box number info to sessions csv file
            continue
        end 
...
%             else
                activeBoxCal = boxCal; % use this instead of activeBoxCal from above line
%             end
