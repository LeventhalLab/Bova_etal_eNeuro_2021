function mean_aperture_traj = breakDownFullApertureByOutcome2(reachData,z_interp_digits,validTrialOutcomes)
...
mean_aperture_traj = NaN(1,length(z_interp_digits),length(validTrialOutcomes));
...
for i_outcome = 1 : length(validTrialOutcomes)
    ...
    mean_aperture_traj(:,:,i_outcome) = nanmean(aperture_traj(outcomeFlag(:,i_outcome),:));
