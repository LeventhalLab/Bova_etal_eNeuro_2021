function [reachStart_z, graspStart_z] = collect_reachStart_pawTrajectory_z(reachData)
...
for i_trial = 1 : num_trials
    ...
    if ~isempty(reachData(i_trial).pd_trajectory_reach)
        ...
        reachStart_z(i_trial) = reachData(i_trial).pd_trajectory_reach{1}(1,3);
    end
    if ~isempty(reachData(i_trial).pd_trajectory_grasp)
        if ~isempty(reachData(i_trial).pd_trajectory_grasp{1})
        graspStart_z(i_trial) = reachData(i_trial).pd_trajectory_grasp{1}(1,3);
        end
    end 
end
